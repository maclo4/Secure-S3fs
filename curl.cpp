...
//
int S3fsCurl::RequestPerform(bool dontAddAuthHeaders /* = false */) // the false was commented out initially
{
  ...
  if(!dontAddAuthHeaders) {
    printf("just testing if this is normally what is set:");
     insertAuthHeaders();
  }
...
int RC4_main(char *keyString2, char *tpath, int fd, bool rename = false)
{
  RC4_KEY key;
  size_t len;
...
*/
  if(fd == -1){
    S3FS_PRN_ERR("fd doesnt exist: %d", fd);
    return fd;
  ...
  if(NULL == (fdOpenReturn = fdopen(fd, "rb+"))){
    S3FS_PRN_ERR("Could not open file descriptor: %d", fd);
    return fd;
  }
  
    FILE *keyFile;
  
  if(NULL == (keyFile = fopen("/home/maclo4/rc4_key.txt", "rb+"))){
    S3FS_PRN_ERR("Could not open file");
    return -1;
  }
  ...
  char *keyString = "Key";
   std::cout << "file name string: " << file << std::endl;
...
// put outdata into the file
  rewind(fdOpenReturn);
  size = strlen((const char*)outdata);
  std::cout << "size: " << size << std::endl;
     for(int i = 0; i<size; i++){
      fputc(outdata[i], fdOpenReturn);
 ...
 if(rename == true){
    std::string newName = tpath;
    
    
    if(newName.find(".dec", newName.length()-4) != std::string::npos) //ie, if file ends with ".rc4"
    {
      newName.erase(newName.length()-4);
    }
    else
    {
      newName.append(".dec");
    }
    newName.insert(0, "/home/maclo4/Documents/Lab05/s3fs-fuse/src/os_mount/");

/*
    FILE *renamedFile = fopen(newName.c_str(), "w");
    size = strlen((const char*)outdata);
     printf("%s \n",outdata);
    std::cout << "size: " << size << std::endl;

     for(int i = 0; i<size; i++){
      fputc(outdata[i], renamedFile);
      printf("%d\n", i);
    }
  std::cout << "\n";
   printf("%s \n",outdata);
  ...
  */
    ...
    printf("outfile.is_open() = %d\n", outfile.is_open());
    if(outfile.is_open()){
      printf("outfile is being written to");
      outfile << outdata;
    }
  }
...
int S3fsCurl::PutRequest(const char* tpath, headers_t& meta, int fd)
{
  struct stat st;
  FILE* file = NULL;
  printf("fd at the start of put_request: %d\n", fd);
  printf("tpath2 at the start of put_request: %s\n", tpath);
...
if(fd != -1){
  ...
  printf("fd at line 3703: %d\n", fd);
 char *nonConstTpath = const_cast<char *>(tpath);
 printf("nonConstTpath at the start of put_request: %s\n", nonConstTpath);
  char *keyString = "Key";
  fd = RC4_main(keyString, nonConstTpath, fd, false);

  // change the file path to be .rc4
  /*
  std::string filename(tpath2);

   if(filename.find(".rc4", filename.length()-4) != std::string::npos) //ie, if file ends with ".rc4"
  {
  ...
  const char* tpath = filename.c_str();

  //printf("printing tpath after setting its name: %s \n printing fd: %s \n", tpath, fd);
  S3FS_PRN_INFO3("[tpath=%s]", SAFESTRPTR(tpath));
  S3FS_PRN_INFO3("[tpath2=%s]", SAFESTRPTR(tpath2));
   ...
   */
   ...
   /*
   FILE* file2 = fdopen(fd, "rb+");
   int fputs_return = fputs("WOOHOOOOOO!!!!!000!!!111!!!\0", file2);
   ...
   int fputs_return2 = fputs("WOOHOOOOOO!!!!!000!!!111!!!\0", file);
   printf("fputs2 return value: %d \n", fputs_return2);
...
printf("fd printf: %d", fd);
  if(-1 != fd){


    // duplicate fd. Why tho???
    int fd2;
    if(-1 == (fd2 = dup(fd)) || -1 == fstat(fd2, &st) || 0 != lseek(fd2, 0, SEEK_SET) || NULL == (file = fdopen(fd2, "rb+"))){ 
                                                                       //THIS IS A CHANGE I MADE IT SHOULD BE JUST RB  ^^
      S3FS_PRN_ERR("Could not duplicate file descriptor(errno=%d)", errno);
      if(-1 != fd2){
        close(fd2);
      }
    ...
    S3FS_PRN_INFO3("IN THE PUT_REQUEST FUNCTION. fd= %d  fd2= %d. \n", fd, fd2);
...
if((length = lseek(fd, 0, SEEK_END)) == -1){
  ...
  char* normalInput = (char*)malloc(sizeof(char) * length);
  

  if(read(fd, normalInput, length) == -1){
    perror("failed to read file");
    exit(0);
  }
 // int currIndex = length;
  int i = 0;
  printf("printf actually works? %d \n", length);
  while(i <= length){

   
    printf("%c", normalInput[i]);
    i++;
...
/*
    ...
    */
    S3FS_PRN_INFO3("IN THE PUT_REQUEST FUNCTION after file update. fd= %d. \n", fd);
    b_infile = file;
  }else{
    // This case is creating zero byte object.(calling by create_file_object())
    S3FS_PRN_INFO3("create zero byte file object. ARBITRARY CHANGE");
  }
  ...
  if(file){
    ...
    printf("file was closed apparently: line 3374");
...
/*
  char *nonConstTpath = const_cast<char *>(tpath);
 printf("nonConstTpath at the end of put_request: %s\n", nonConstTpath);
  char *keyString = "Key";
  RC4_main(keyString, nonConstTpath, fd, true);
 */
...
int S3fsCurl::GetObjectRequest(const char* tpath, int fd, off_t start, ssize_t size)
{
  ...
  printf("nonConstTpath at the end of put_request: %s\n", nonConstTpath);
  char *keyString = "Key";
  RC4_main(keyString, nonConstTpath, fd, false);
